...
namespace NodeCanvas.Framework
{
    partial class Node
    {
        ...
        [SerializeField] private VerboseLevel _verboseLevel = VerboseLevel.Full;
            ...
            if (!node.showIcon || node.iconAlignment != Alignment2x2.Default)
            {
                if (node.name != null)
                {
                    ...
                    if (node is StateNode stateNode)
                        if (ScenarioManager.current == stateNode)
                        {
                            GUI.color = ScenarioManager.currentNodeColor;
                            var headerHeight = stateNode.rect.height <= 35 ? 35 : 27;
                            GUI.Box(new Rect(0, 0, stateNode.rect.width, headerHeight), string.Empty,
                                StyleSheet.windowHeader);
                            GUI.color = Color.black;
                        }
                        else if (ScenarioManager.other == stateNode)
                        {
                            GUI.color = ScenarioManager.otherNodeColor;
                            var headerHeight = stateNode.rect.height <= 35 ? 35 : 27;
                            GUI.Box(new Rect(0, 0, stateNode.rect.width, headerHeight), string.Empty,
                                StyleSheet.windowHeader);
                            GUI.color = Color.black;
                        ...
                        else
                        {
                            if (node.nodeColor != default(Color))
                            {
                                GUI.color = ScenarioManager.defaultNodeColor;
                                var headerHeight = node.rect.height <= 35 ? 35 : 27;
                                GUI.Box(new Rect(0, 0, node.rect.width, headerHeight), string.Empty,
                                    StyleSheet.windowHeader);
                                GUI.color = Color.black;
                            }
                        }
        ...
        static GenericMenu GetNodeMenu_Multi(Graph graph)
        {
            ...
            menu.AddItem(new GUIContent("Copy Selected Nodes"), false,
                () =>
                {
                    CopyBuffer.Set<Node[]>(Graph.CloneNodes(GraphEditorUtility.activeElements.OfType<Node>().ToList())
                        .ToArray());
                });
            ...
            if (node is ITaskAssignable)
            {
                ...
                if (CopyBuffer.Has<Task>())
                {
                    menu.AddItem(new GUIContent("Paste Assigned Task"), false, () =>
                    {
                        ...
                        if (assignable.task != null)
                        {
                            if (!EditorUtility.DisplayDialog("Paste Task",
                                string.Format(
                                    ...
                                    assignable.task.name, CopyBuffer.Peek<Task>().name), "YES", "NO"))
                            {
                                return;
                            }
                        }
                        ...
                        try
                        {
                            assignable.task = CopyBuffer.Get<Task>().Duplicate(node.graph);
                        }
                        catch
                        {
        ...
        static void DrawNodeTag(Node node)
        {
            if (!string.IsNullOrEmpty(node.tag))
            {
                ...
                var tagRect = new Rect(node.rect.x + 40 , node.rect.y-20, size.x, size.y);
                ...
                tagRect.y += tagRect.height - 15;
                tagRect.x = node.rect.x + 20;
        ...
        virtual public void OnActiveRelinkEnd(Connection connection)
        {
            for (var i = 0; i < graph.allNodes.Count; i++)
            {
                ...
                if (otherNode != connection.targetNode && otherNode != connection.sourceNode &&
                    otherNode.rect.Contains(Event.current.mousePosition))
                {
