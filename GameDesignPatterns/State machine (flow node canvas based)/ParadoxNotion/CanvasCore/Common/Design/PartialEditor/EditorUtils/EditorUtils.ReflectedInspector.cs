...
namespace ParadoxNotion.Design
{
    ...
    partial class EditorUtils
    {
        ...
        {
            List<MethodInfo> MethodsToShow = new List<MethodInfo>();
            ...
            {
                var customAttributeDatas = method.CustomAttributes
                    .Where(x => x.AttributeType == typeof(ShowInGraphNodeInspector))
                    .Select(x => x).ToList();
                if (customAttributeDatas.Any())
                {
                    MethodsToShow.Add(method);
                }
            }

            foreach (var methodInfo in MethodsToShow)
            {
                var s = methodInfo.GetCustomAttributes(typeof(ShowInGraphNodeInspector));
                var name = "";
                foreach (var attribute in s)
                {
                    if (attribute is ShowInGraphNodeInspector)
                    {
                        var a = attribute as ShowInGraphNodeInspector;
                        name = a.name;
                    }
                }
            ...
            foreach (var field in fields)
            {
                ...
                if (field.IsPublic || field.IsDefined(typeof(ExposeFieldAttribute), true))
                {
                    ...
                    field.SetValue(target,
                        ReflectedFieldInspector(field.Name, field.GetValue(target), field.FieldType, field, target,
                            attributes));
                }
            }
        }
        ...
        public static object ReflectedFieldInspector(string name, object value, Type t, FieldInfo field = null,
            object context = null, object[] attributes = null)
        {
        ...
        ///Show an arbitrary field type editor. Passing a FieldInfo will also check for attributes.
        public static object ReflectedFieldInspector(GUIContent content, object value, Type t, FieldInfo field = null,
            object context = null, object[] attributes = null)
        {
            if (t == null)
            {
            ...
            ///Use drawers
            var drawerAttributes = attributes != null
                ? attributes.OfType<DrawerAttribute>().OrderBy(a => a.priority).ToArray()
                : null;
