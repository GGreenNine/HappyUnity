...
namespace FlowCanvas.Nodes
{
    ...
    public class ReflectedDelegateEvent
    {
        ...
        MethodInfo GetMethodForDelegateType(Type delegateType) {
            ...
            if ( parameters.Length == 0 ) { result = thisType.GetMethod("Callback0"); } else if ( parameters.Length == 1 ) { result = thisType.GetMethod("Callback1"); } else if ( parameters.Length == 2 ) { result = thisType.GetMethod("Callback2"); } else if ( parameters.Length == 3 ) { result = thisType.GetMethod("Callback3"); } else if ( parameters.Length == 4 ) { result = thisType.GetMethod("Callback4"); } else if ( parameters.Length == 5 ) { result = thisType.GetMethod("Callback5"); } else if ( parameters.Length == 6 ) { result = thisType.GetMethod("Callback6"); } else if ( parameters.Length == 7 ) { result = thisType.GetMethod("Callback7"); } else if ( parameters.Length == 8 ) { result = thisType.GetMethod("Callback8"); } else if ( parameters.Length == 9 ) { result = thisType.GetMethod("Callback9"); } else if ( parameters.Length == 10 ) { result = thisType.GetMethod("Callback10"); }
            try {
